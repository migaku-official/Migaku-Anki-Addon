...
function parseSyntaxEuropeanWithGender(text) {
    let ret = [];
    const syntax_re = /(?:\(([a-zA-Z\u00C0-\u00D6\u00D8-\u00F6\u00F8-\u024F']+?)\)|([a-zA-Z\u00C0-\u00D6\u00D8-\u00F6\u00F8-\u024F']+?))\[(.*?)\]/gm;
    let last_idx = 0;
    let match;
...
if (runTest('Italian: New format with parentheses and pipe alternatives', () => {
    const input = '(Ho)[avere,verb,m|havere,verb,m] (salutato)[salutare,verb,m]';
...
if (runTest('Italian: Old format without parentheses', () => {
    const input = 'Ho[avere,verb,m] salutato[salutare,verb,m]';
    const result = parseSyntaxEuropeanWithGender(input);
    
    assertEquals(result.length, 3, 'Should parse into 3 elements');
    assertEquals(result[0].text, 'Ho', 'Should extract word without parentheses');
    assertEquals(result[0].dict_form, 'avere', 'Should extract lemma');
})) passed++; else failed++;

if (runTest('Italian: Contractions with apostrophes', () => {
    const input = "(d')[di,adp]acqua";
    const result = parseSyntaxEuropeanWithGender(input);
    
    assertEquals(result.length, 2, 'Should parse into 2 elements');
    assertEquals(result[0].text, "d'", "Should handle apostrophe in contraction");
...
if (runTest('Italian: Complex sentence with multiple forms', () => {
    const input = '(il)[il,art,m] (mio)[mio,det,m|mio,pron,m] (amico)[amico,noun,m|amico,adj,m|amicare,verb]';
    const result = parseSyntaxEuropeanWithGender(input);
    
    assertEquals(result[0].text, 'il', 'Should parse article');
    assertEquals(result[2].text, 'mio', 'Should parse determiner');
    assertEquals(result[2].dict_form, 'mio', 'Should use first alternative for mio');
    assertEquals(result[2].word_pos, 'det', 'Should use first POS');
    assertEquals(result[4].text, 'amico', 'Should parse noun');
    assertEquals(result[4].word_pos, 'noun', 'Should prefer noun over adjective/verb');
})) passed++; else failed++;

if (runTest('Italian: Mixed old and new format', () => {
    const input = '(Ho)[avere,verb] mangiato[mangiare,verb] (la)[il,art,f] pasta';
    const result = parseSyntaxEuropeanWithGender(input);
...
if (runTest('Italian: Plain text without syntax', () => {
...
if (runTest('French: Contractions with apostrophes', () => {
...
if (runTest('French: Gender markers', () => {
    const input = '(le)[le,art,m] (chat)[chat,noun,m] (et)[et,conj] (la)[le,art,f] (maison)[maison,noun,f]';
    const result = parseSyntaxEuropeanWithGender(input);
...
if (runTest('English: With IPA pronunciation', () => {
...
if (runTest('English: Multiple IPA alternatives', () => {
...
if (runTest('English: Old format backward compatibility', () => {
...
if (runTest('Only pipes (malformed alternatives)', () => {
    const input = '(word)[|||]';
    const result = parseSyntaxEuropeanWithGender(input);
...
if (runTest('Special characters in plain text', () => {
    const input = 'Hello, world! How are you?';
    const result = parseSyntaxEuropeanWithGender(input);
    
    assertEquals(result.length, 1, 'Should treat as plain text');
    assertEquals(result[0].text, 'Hello, world! How are you?', 'Should preserve punctuation');
})) passed++; else failed++;

if (runTest('Accented characters', () => {
    const input = '(café)[café,noun,m] (naïve)[naïve,adj]';
    const result = parseSyntaxEuropeanWithGender(input);
    
    assertEquals(result[0].text, 'café', 'Should handle accented chars in word');
    assertEquals(result[0].dict_form, 'café', 'Should handle accented chars in metadata');
