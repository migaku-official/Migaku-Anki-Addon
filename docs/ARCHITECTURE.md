...
Ephemeral media and conversion files are stored in a process-scoped directory under the operating system's temporary location, not inside the installed add-on directory. This keeps active audio files from preventing Anki from replacing the add-on during an update. Each media conversion uses an isolated operation workspace so concurrent submissions cannot overwrite or delete one another's files. The temporary-storage boundary recreates its process directory if the operating system removes it while Anki is still running.
...
The modern card uplift establishes an explicitly reviewed functional baseline in
addition to its visual changes:

- Front templates use field-presence sections around the selected Sentence or
  Target Word. An empty selected text field therefore produces no empty editable
  container.
- Back templates reveal Translation through a one-time local disclosure without
  changing note data.
- Back templates expose the four existing card types through independent
  text/audio and sentence/vocabulary switches. Each switch change continues to
  persist through the existing `update_card_type` command.
- Managed base note types use collision-resistant WOFF2 media names generated by
  the font importer instead of the legacy `_NotoSerif*.ttf` names.

These behaviors and media references are intentional contract state, not
accidental cosmetic drift. The approved hashes in
`dev/card-preview/template-contract.json` lock this reviewed baseline for future
changes.
...
the current language but does not delete collection media, because customized
note types may retain CSS references to legacy filenames. Upgraded collections
can therefore keep serving legacy custom note types while managed base note
types move to the namespaced WOFF2 assets. New migrations must preserve that
compatibility rule unless they include an explicit custom-note-type migration.
