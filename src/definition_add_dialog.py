...
class DefinitionAddDialog(QDialog):
    ...
    def __init__(self, lang: Language, note_type: NotetypeDict, note_ids, parent=None):
        ...
        lbl = QLabel('<b>Note:</b> The dictionaries that are searched can be configured in the "Automatic definition settings" at the bottom of the card creator of the Migaku dictionary.')
    ...
    def start(self):
        ...
        self.config['mapping']['example_sentences'] = self.out_box_example_sentences.currentText()
        ...
        self.checkpoint_id = aqt.mw.col.add_custom_undo_entry(F'Added {self.lang.name_en} Definitions')
        self.current_idx = 0
    ...
    def on_batch_delivery(self, batch):
        ...
        CollectionOp(self, self.handle_batch_delivery_data).success(self.on_batch_notes_flushed).run_in_background()
    ...
    def handle_batch_delivery_data(self, col):
        ...
        for note in self.current_batch_notes:
            ...
            if note_id_str in batch:
                for key, mapping in [('definitions', 'text'), ('googleImages', 'images'), ('forvo', 'word_audio'), ('tatoeba', 'example_sentences')]:
