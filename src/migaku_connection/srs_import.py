...
class SrsImportInfoHandler(MigakuHTTPHandler):
    ...
    def get(self):
        ...
        for deck_info in col.decks.all_names_and_ids(skip_empty_default=True, include_filtered=False):
...
class SrsSampleCardHandler(MigakuHTTPHandler):
    ...
    async def post(self):
        ...
        if cardIds:
            ...
            frontHtml = '<!DOCTYPE html><body>' + self.AUDIO_HTML + self.fix_av(self.fix_src(card.question()), card.question_av_tags()) + '</body></html>'
            backHtml =  '<!DOCTYPE html><body>' + self.AUDIO_HTML + self.fix_av(self.fix_src(card.answer()),   card.answer_av_tags())   + '</body></html>'
