    ...
    def load_tmpl(self):
        for fmt, list_widget in [("qfmt", self.qlist), ("afmt", self.alist)]:
            ...
            if self.current_lang:
                ...
                list_widget.setHorizontalHeaderLabels(
                    ["Field"] + [f.label for f in self.current_lang.field_settings]
                )
            else:
            ...
            fields_settings = note_type_mgr.nt_get_tmpl_fields_settings(
                self.model, self.current_tmpl_idx, fmt, field_names=True
            )
                ...
                if self.current_lang:
                    ...
                    if field_active:
                        for i, s in enumerate(self.current_lang.field_settings):
                            setting_box = list_widget.cellWidget(rid, i + 1)
                            field_settings[s.name] = s.options[
                                setting_box.currentIndex()
                            ].value
    ...
    def accept(self):
        ...
        if not selected_item:
            QMessageBox.information(
                self,
                "Error",
                "Please select a language for which you want to add the Migaku card type.",
            )
            return
