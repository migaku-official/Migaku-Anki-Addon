...
def infer_migaku_type(name: str) -> str:
    ...
    if re.search(
        ...
        r"(photo|写真|照片|사진|foto|foto|photo|foto|foto)", name, re.IGNORECASE
    ) or re.search(
        r"(drawing|絵|图画|그림|dibujo|desenho|dessin|zeichnung|desenho)", name, re.IGNORECASE,
    ) or re.search(
        r"(screenshots|スクリーンショット|截图|스크린샷|capturas de pantalla|capturas de tela|captures d'écran|bildschirmfotos|capturas de tela)", name, re.IGNORECASE,
    ):
        return "image"
    if re.search(r"(sentence|文|句|문장|frase|phrase|satz|frase)", name, re.IGNORECASE):
        return "sentence"
